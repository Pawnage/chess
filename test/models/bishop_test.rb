...
class BishopTest< ActiveSupport::TestCase
  ...
  test "#obstructed_move? for all four directions with pieces existing in between current position and destination" do
    ...
    pawn_top_right = FactoryGirl.create(:pawn, :game_id => game.id, :row_position => 5, :col_position => 5)
    pawn_bottom_right = FactoryGirl.create(:pawn, :game_id => game.id, :row_position => 1, :col_position => 5)
    pawn_top_left = FactoryGirl.create(:pawn, :game_id => game.id, :row_position => 4, :col_position => 2)
    pawn_bottom_left = FactoryGirl.create(:pawn, :game_id => game.id, :row_position => 2, :col_position => 2)

    assert bishop.obstructed_move?(7, 7) # move upward and rightward
    assert bishop.obstructed_move?(0, 6) # move downward and rightward
    assert bishop.obstructed_move?(5, 1) # move upward and leftward
